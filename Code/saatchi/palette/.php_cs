...
if ($hasPedroTroller) {
    $rules = \array_merge($rules, [
        // THIRD_PARTY FIXERS
        // 'PedroTroller/order_behat_steps' => true, // Step definition methods in Behat contexts MUST BE ordered by annotation and method name.
        // 'PedroTroller/ordered_with_getter_and_setter_first' => true, // Class/interface/trait methods MUST BE ordered (accessors at the beginning of the class, ordered following properties order).
        // 'PedroTroller/line_break_between_statements' => true, // Each statement (in, for, foreach, ...) MUST BE separated by an empty line
        // 'PedroTroller/useless_comment' => true, // Comments that do not provide more precision than the definition of a method MUST BE deleted DEPRECATED
        'no_superfluous_phpdoc_tags' => true, // Comments that do not provide more precision than the definition of a method MUST BE deleted
        'PedroTroller/line_break_between_method_arguments' => ['max-length' => 90, 'automatic-argument-merge' => true], // 'max-args' => 4,
        'PedroTroller/useless_code_after_return' => true, // All return that are not accessible (i.e. following another return) MUST BE deleted
        'PedroTroller/comment_line_to_phpdoc_block' => true,
        'PedroTroller/exceptions_punctuation' => true, // Exception messages MUST ends by ".", "…", "?" or "!".
        'PedroTroller/forbidden_functions' => ['functions' => ['var_dump', 'dump', 'dd']], // Prohibited functions MUST BE commented on as prohibited
